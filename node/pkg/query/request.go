...
func (saq *SolanaAccountQueryRequest) AccountList() []string {
	return saq.Accounts
}
...
func (saq *SolanaAccountQueryRequest) Marshal() ([]byte, error) {
	...
	for _, acct := range saq.Accounts {
		vaa.MustWrite(buf, binary.BigEndian, uint32(len(acct)))
		buf.Write([]byte(acct))
...
func (saq *SolanaAccountQueryRequest) UnmarshalFromReader(reader *bytes.Reader) error {
	...
	for count := 0; count < int(numAccounts); count++ {
		len := uint32(0)
		if err := binary.Read(reader, binary.BigEndian, &len); err != nil {
			return fmt.Errorf("failed to read account len: %w", err)
		}
		account := make([]byte, len)
		if n, err := reader.Read(account[:]); err != nil || n != int(len) {
			return fmt.Errorf("failed to read account [%d]: %w", n, err)
		}

		saq.Accounts = append(saq.Accounts, string(account))
	}
...
func (saq *SolanaAccountQueryRequest) Validate() error {
	...
	for _, acct := range saq.Accounts {
		if len(acct) > math.MaxUint32 {
			return fmt.Errorf("account is too long")
		}
		_, err := base58.Decode(acct)
		if err != nil {
			return fmt.Errorf(`account "%s" is invalid base58`, acct)
		}
	}
...
func (left *SolanaAccountQueryRequest) Equal(right *SolanaAccountQueryRequest) bool {
	...
	for idx := range left.Accounts {
		if left.Accounts[idx] != right.Accounts[idx] {
			return false
		}
	}
